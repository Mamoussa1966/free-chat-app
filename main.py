...
def _render_sidebar(rounds: int, credentials: dict, model_candidates: dict) -> int:
    with st.sidebar:
        ...
        for seat in SEATS:
            models = model_candidates.get(seat.key) or (seat.default_model, *seat.fallback_models)
...
def _render_diagnostics(results: list[dict], title: str = "🔎 التشخيص والنتائج") -> None:
    official = sum(r["status"] == "SUCCESS" for r in results)
    failed = sum(r["status"] == "FAILED" for r in results)
    st.subheader(title)
    st.info(
        f"آخر عملية: {official}/5 استجابات رسمية • فشل: {failed} • Local Engine: غير مستخدم"
    )

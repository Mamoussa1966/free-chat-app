...
def _shared_context(chat: dict, max_chars: int = 30000) -> str:
    lines = []

    for index, item in enumerate(chat["messages"]):
        if index == len(chat["messages"]) - 1 and item.get("role") == "user":
            continue
        ...
        if item.get("role") == "user":
            ...
            lines.append(
                f"{item.get('seat', 'AI')}: {item.get('content', '')}"
            )
...
def _render_user_room(chat: dict) -> None:
    with st.container(
        ...
        if not user_messages:
            st.caption(
                "اكتب رسالة أو أرفق صورة/ملف/مجلد "

# AI Council V22.1 — 20-Provider Free API Cascade — Final Hotfix 3

This final hotfix keeps the V22.1 20-provider registry intact and hardens runtime error classification/cascade handling: the package exposes **20 provider seats** and routes each configured seat through an explicit ordered Free API cascade of up to 10 model IDs.
...
The package also includes an independent `gitops_layer.py` security/policy layer and `tests/test_gitops_layer.py` with 39 adversarial regression tests. GitOps is **not wired into the Streamlit runtime and does not perform GitHub pushes** in this release. The layer is disabled by architecture: there is no automatic AI-to-GitHub path, and the simulated push boundary requires explicit human approval and verifies hash bindings/TOCTOU conditions.

This layer is an engineering hardening boundary, not a substitute for OS/container sandboxing, durable proposal storage, trusted authentication, or GitHub branch protection.
...
API keys are read only from Streamlit Secrets or environment variables. Keys are never rendered in the UI, persisted in chat history, or included in provider error messages. Error payloads are sanitized before being shown.

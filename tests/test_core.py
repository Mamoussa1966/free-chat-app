...
class CoreTests(unittest.TestCase):
    def test_five_core_seats(self):
        ...
        self.assertEqual(
            ...
            ["openai", "gemini", "claude", "grok", "kimi"],
        )

    def test_error_classification(self):
        self.assertEqual(
            _classify(401, "invalid api key"),
            "authentication_or_permission",
        )
        self.assertEqual(
            _classify(429, "quota exceeded"),
            "rate_limit_or_quota",
        )
        self.assertEqual(
            _classify(404, "model not found"),
    ...
    def test_all_defaults_are_nonempty(self):
        for seat in SEATS:
            self.assertTrue(seat.default_model)
            self.assertTrue(seat.fallback_models)

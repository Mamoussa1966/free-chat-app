...
def call_seat(
    ...
    for index, model in enumerate(candidates):
        ...
        try:
            ...
            # This is the authoritative execution identity: the exact model
            # argument used for the successful provider request.
            executed_model = model
            return _result(
                seat, "SUCCESS", "official", model, content, None,
                started, attempted, executed_model=executed_model,
            )
        except ProviderError as exc:
            last_error = exc
...
def _result(seat: Seat, status: str, mode: str, model: str, content: str, error: Optional[str], started: float, attempted: list[str], executed_model: Optional[str] = None) -> dict:
    """Build one immutable-ish result contract.

    ``executed_model`` is the exact model ID handed to ``call_official`` for
    the successful attempt.  For a successful official call it MUST equal
    ``model``; the UI uses ``model`` only after this invariant is verified.
    """
    if status == "SUCCESS" and mode == "official":
        if not executed_model or executed_model != model:
            raise ProviderError(
                "model execution identity invariant violated",
                error_class="model_execution_identity_mismatch",
            )
    return {
        "seat": seat.key,
        "name": seat.name,
        ...
        "mode": mode,
        "model": model,
        "executed_model": executed_model or "",
        "content": content,
        "error": error,
        "latency": round(time.perf_counter() - started, 3),

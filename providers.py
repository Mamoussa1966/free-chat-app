...
# IMPORTANT:
# All Seat definitions use keyword arguments to prevent positional
# argument drift and constructor mismatch errors.
SEATS = (
    Seat(
        key="openai",
        name="ChatGPT",
    ...
    Seat(
        ...
        # Kept only as legacy metadata for compatibility with existing tests
        # and older configuration introspection.
        #
        # IMPORTANT: get_model_candidates() DOES NOT automatically use these.
        # Gemini models must be explicitly supplied through GEMINI_FREE_MODELS.
...
def get_model_candidates(seat: Seat) -> Tuple[str, ...]:
    """Return only explicitly configured Free API models.

    STRICT CONTRACT
    ---------------
    The council never automatically selects a model.

    A model is eligible only when its identifier is explicitly supplied
    through the seat's *_FREE_MODELS configuration.

    The legacy default_model/fallback_models fields are intentionally NOT
    used here. They remain only for backward compatibility with older
    metadata/tests.

    Empty configuration means:
        - no official model is available for this seat;
        - no paid model is selected;
        - no Local Engine is used.
    """
...
def _openai_models_probe(
    ...
    """Validate OpenAI authentication independently of model selection.

    The probe deliberately calls GET /v1/models instead of a generation
    endpoint. This allows diagnostics to distinguish:

        valid authentication + no configured Free model

    from:

        invalid/blocked OpenAI credential.
    """
...
def _prompt(
    ...
    # Shared context may contain user attachments or previous model output.
    # It is treated as untrusted data, never as higher-priority instructions.

    context = (
        str(shared_context or "")
        .strip()
...
def transcribe_audio_gemini(
    ...
    This is a utility path, not an AI-council seat.

    It never uses Local Engine as a fake transcription service.
    """
                    ...
                    "contents": [
                        {
                            ...
                            "parts": [
                                {
                                    "text": (
                                        "Transcribe the attached audio exactly "
                                        "as spoken. Return only the transcription. "
                                        "Preserve Arabic and English words, "
                                        "numbers, and names. Do not summarize "
                ...
                content.append(
                    {
                        "type": "input_text",
                        "text": (
                            "Attached file omitted from inline API payload "
                            "because it exceeds the provider payload safety cap: "
...
def call_seat(
    ...
    local_fallback remains in the signature only for compatibility with
    older callers. It is deliberately ignored.

    There is NO Local Engine path.
    """
    del local_fallback
...
def diagnostic_seat(
    ...
    """Run an independent official-provider diagnostic."""
    ...
    if seat.key == "openai":
        ...
        if not candidates:
            return {
                ...
                "error": (
                    ...
                    "OpenAI API authentication succeeded, but no Free API "
                    "model is configured for ChatGPT. "
        ...
        # The independent /v1/models probe established authentication.
